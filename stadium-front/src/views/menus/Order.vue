    ...
    <div class="main">
        <div class="search">
            <el-form :model="searchRef" ref="searchFormRef" :inline="true">
                ...
                <el-form-item label="教练姓名：" prop="coId">
                    <el-select v-model="searchRef.coId" placeholder="请选择教练" clearable style="width: 120px" filterable>
                        <el-option key="0" label="不限" value="" />
                        <el-option v-for="co in coachList" :key="co.id" :label="co.coName" :value="co.id" />
                    </el-select>
                </el-form-item>
                ...
                <br />
                <el-form-item label="预约时间段：" prop="timeRange">
                    <el-date-picker v-model="searchRef.timeRange" type="datetimerange" start-placeholder="开始时间"
                    value-format="YYYY-MM-DD HH:mm:ss" end-placeholder="结束时间" :default-time="defaultTime" />
                </el-form-item>
                <br />
                <el-form-item>
                    <el-button disabled @click="add" type="success" :icon="Plus" plain>新增</el-button>
                    <el-button @click="reset" type="info" :icon=Refresh plain>重置</el-button>
                    <el-button disabled @click="del" type="danger" :icon="Delete" plain>批量删除</el-button>
        ...
        <div class="data-grid">
            <el-table :data="tableData" ref="tableRef" v-loading="loading" element-loading-text="Loading..."
                ...
                <el-table-column prop="user.avatar" label="用户头像" width="140">
                    <template #default="scope">
                        <el-image class="row-avatar" :src="scope.row.user.avatar"
                            @click="showPic(scope.row.user.avatar)" />
                    </template>
                </el-table-column>
                ...
                <el-table-column prop="area.areaName" label="预约场地" width="180" />
                <el-table-column prop="coach.coName" label="预约陪练" width="100">
                    <template #default="scope">
                        <span v-if="scope.row.coach">{{ scope.row.coach.coName }}</span>
                        <span v-else>无</span>
                    </template>
                </el-table-column>
                <el-table-column prop="startTime" label="开始时间" width="150" />
                <el-table-column prop="endTime" label="结束时间" width="150" />
                <el-table-column prop="payment" label="订单总金额" width="150">
                    <template #default="scope">
                        <span>{{ scope.row.payment }} 元</span>
                    </template>
                </el-table-column>
                <el-table-column prop="phone" label="预定人手机号" width="150" show-overflow-tooltip />
                ...
                <el-table-column fixed="right" label="操作" min-width="200">
                    <template #default="scope">
                        ...
                        <el-button disabled link type="success" @click="edit(scope.row)">
                            编辑
                        </el-button>
                        <el-button disabled link type="danger" @click="delInLine(scope.row)">
                            删除
                        </el-button>
                    </template>
                </el-table-column>
        ...
        <el-drawer v-model="showDrawer" show-close title="详情" direction="rtl" size="30%">
            <div class="drawer-pic">
                <el-image class="drawer-avatar" :src="rowData.user.avatar" />
            </div>
            <el-descriptions :title="rowData.nickname" direction="vertical" border>
                ...
                <el-descriptions-item label="预约场地">
                    <span>{{ rowData.area.areaName }}</span>
                </el-descriptions-item>
                <el-descriptions-item label="结束时间">
                    <span>{{ rowData.endTime }}</span>
                </el-descriptions-item>
                <el-descriptions-item label="预约陪练">
                    <span>{{ rowData.coach.coName }}</span>
                </el-descriptions-item>
                <el-descriptions-item label="订单总金额">
                    <span>{{ rowData.payment }} 元</span>
                </el-descriptions-item>
                <el-descriptions-item label="手机号">
                    <span>{{ rowData.phone }}</span>
                </el-descriptions-item>
                <el-descriptions-item label="订单状态">
                    <span v-if="rowData.status === '0'">预约中</span>
                    <span v-if="rowData.status === '1'">已完成</span>
                    <span v-if="rowData.status === '2'">已取消</span>
                </el-descriptions-item>
        ...
        <el-dialog v-model="showDialog" :title="dialogTit" width="800" draggable :close-on-click-modal="false"
            ...
            <el-from :model="formMod" :rule="rules" ref="formRef">
                <el-row>
                    <el-col :span=12>
                        <el-form-item label="昵称：" label-width="100" prop="nickname">
                            <el-input v-model="formMod.nickname" placeholder="起个好听的名字" />
                        </el-form-item>
                ...
                <el-row v-show="actionRef">
                    <el-col :span="12">
                        <el-form-item label="用户名：" label-width="100" prop="username">
                            <el-input v-model="formMod.username" placeholder="输入用户名" />
                        </el-form-item>
                    </el-col>
                    <el-col :span="12">
                ...
                <el-row>
                    <el-col :span="12">
                        <el-form-item label="QQ：" label-width="100" prop="qq">
                            <el-input v-model="formMod.qq" placeholder="请输入QQ" />
                        </el-form-item>
                    </el-col>
                    <el-col :span="12">
                        <el-form-item label="微信：" label-width="100" prop="wechat">
                            <el-input v-model="formMod.wechat" placeholder="请输入微信" />
                        </el-form-item>
                    </el-col>
                </el-row>
...
<script setup>
import { getList as apiSearch, add as apiAdd, edit as apiEdit, deleteByIds as apiDeleteByIds } from '@/api/orderApi'
// import { getIdsByName as apiGetUserIds } from '@/api/userApi'
import { getList as apiGetCoach } from '@/api/coachApi'
...
async function getCoach() {
    let resp = await apiGetCoach()
    coachList.value = resp.data
...
//提交编辑
async function submitAdd(user) {
    let resp = await apiAdd(user)
...
//提交编辑
async function subimtEdit(user) {
    let resp = await apiEdit(user)
    if (resp.success) {
        ElMessage({
            message: '操作成功',
            type: 'success'
